...
//                var ret = prependScriptBody(node, body);
    ...
    function addScopes(ast) {
        ...
        var visitorPre = {
            ...
            // Markus: need to the scope information at variable declarations
            // to see if it is top-level
            'VariableDeclaration': function(node) {
              node.scope = currentScope;
            },
...
//        console.log(JSON.stringify(newAst,null,"  "));
  ...
  //
  // This is used to hoist such variable declarations in code generated by
  // eval(). We need this because we (the fuzzer) wrap all script code in a
  // closure so it can be moved around. So, the variable declarations which
  // were to the global scope become parts of the scope of the closure.
  //
  // Note that this does not actually wrap the function in a closure.
  ...
  var hoistVarDec = function(src, node) {
    ...
    if (assignments.length) {
      ...
      var trailingSemicolon = node.parent 
                              && node.parent.type === 'ForStatement' 
                              && node === node.parent.init 
                              ? '' : ';';
      //node.update(assignments.join(', ') + trailingSemicolon);
      update(node, chunks, assignments.join(', ') + trailingSemicolon);
    } 
    else if ((node.parent && node.parent.type === 'ForInStatement')
  ...
  var hoistFuncDec = function(src, node) {
    ...
    if (!(node.scope.parent.parent == null)) {
      console.log("node:", node);
      console.log("   not top level");
      return node;
    }
        ...
        if (!skip && typeof code === 'string' && code.indexOf(noInstr) < 0) {
            try {
                ...
                if (isEval && newAst.scope && newAst.scope.parent == null && !options.allowReturnOutsideOfFunction) {
                  ...
                  newAst  = astUtil.transformAst(newAst, visitor, undefined, astUtil.CONTEXT.RHS);
                  code = esotope.generate(newAst, {comment: true});
                }
